...
from custom_components.maicows.maico_ws_api import MaicoWS, MaicoWSRegisters
...
def mock_modbus_client():
    ...
    with patch(
        ...
        # But wait, MagicMock isn't awaitable by default unless used as return_value of an AsyncMock
...
async def test_write_ventilation_level(mock_modbus_client):
    ...
    # Note: write_register is now a coroutine side_effect, checking calls on the mock_client itself might be tricky
    # heavily mocked objects sometimes loose call history on side_effect coroutines depending on how they are attached
